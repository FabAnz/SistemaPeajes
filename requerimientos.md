...
* **Escenario:** Ingreso exitoso
    * **DADO** que soy un propietario registrado con estado "Habilitado", "Suspendido" o "Penalizado" [cite: 43, 46, 47]
    * **CUANDO** ingreso mi cédula de identidad y mi contraseña correctas [cite: 74]
    * **ENTONCES** el sistema me muestra el "Tablero de control del propietario". [cite: 75]
...
* **Escenario:** Credenciales incorrectas
    ...
    * **ENTONCES** el sistema muestra el mensaje "Acceso denegado". [cite: 77]

* **Escenario:** Propietario deshabilitado
    * **DADO** que soy un propietario registrado con estado "Deshabilitado" [cite: 44]
    * **CUANDO** ingreso mi cédula de identidad y mi contraseña correctas
    * **ENTONCES** el sistema muestra el mensaje "Usuario deshabilitado, no puede ingresar al sistema". [cite: 78]
...
* **Escenario:** Visualización de información completa en el tablero
    ...
    * **ENTONCES** el sistema me muestra:
        * Mi nombre completo, estado y saldo actual. [cite: 145, 146, 147]
        * Una tabla de bonificaciones asignadas (nombre, puesto, fecha de asignación). [cite: 148]
        * Una tabla de vehículos registrados (matrícula, modelo, color, cantidad de tránsitos, monto total gastado). [cite: 149, 150]
        * Una tabla de tránsitos realizados ordenada por fecha/hora descendente (puesto, matrícula, categoría, monto tarifa, bonificación, monto bonificación, monto pagado, fecha, hora). [cite: 151]
        * Una tabla de notificaciones del sistema ordenada por fecha/hora descendente (fecha, hora, mensaje). [cite: 152]
...
* **Escenario:** Borrado de notificaciones exitoso
    ...
    * **CUANDO** hago clic en la opción "Borrar notificaciones" [cite: 153]
    * **ENTONCES** el sistema elimina todas mis notificaciones. [cite: 153]
...
* **Escenario:** Intento de borrado sin notificaciones
    ...
    * **ENTONCES** el sistema me muestra el mensaje "No hay notificaciones para borrar". [cite: 155]
...
* **Escenario:** Ingreso exitoso
    ...
    * **CUANDO** ingreso mi cédula de identidad y contraseña correctas [cite: 169]
    * **ENTONCES** el sistema me muestra el menú principal de administrador. [cite: 170]

* **Escenario:** Credenciales incorrectas
    * **DADO** que soy un usuario intentando ingresar a la aplicación de administradores
    * **CUANDO** ingreso una cédula de identidad y/o contraseña incorrecta
    * **ENTONCES** el sistema muestra el mensaje "Acceso denegado". [cite: 172]
...
* **Escenario:** Administrador ya autenticado
    ...
    * **ENTONCES** el sistema muestra el mensaje "Ud. Ya está logueado". [cite: 173]
...
* **Escenario:** Tránsito emulado exitosamente
    ...
    * **CUANDO** ingreso la matrícula, fecha/hora y hago clic en "Emular tránsito" [cite: 205]
    * **ENTONCES** el sistema registra el tránsito, descuenta el monto del saldo del propietario, le genera una notificación y muestra en pantalla el resultado con el nuevo saldo. [cite: 206, 207, 212, 213]
...
* **Escenario:** Matrícula no encontrada
    ...
    * **ENTONCES** el sistema muestra el mensaje "No existe el vehículo". [cite: 215]
...
* **Escenario:** Saldo insuficiente
    ...
    * **ENTONCES** el sistema no registra el tránsito y muestra el mensaje "Saldo insuficiente:" seguido del saldo actual. [cite: 216]

* **Escenario:** Propietario deshabilitado o suspendido
    * **DADO** que el propietario del vehículo está en estado "Deshabilitado" o "Suspendido" [cite: 217, 218]
    * **CUANDO** intento emular el tránsito
    * **ENTONCES** el sistema no registra el tránsito y muestra el mensaje correspondiente. [cite: 217, 218]
...
* **Escenario:** Asignación de bonificación exitosa
    * **DADO** que he buscado un propietario por su cédula y este no tiene una bonificación asignada para el puesto seleccionado [cite: 244]
    * **CUANDO** selecciono una bonificación, un puesto y hago clic en "Asignar bonificación" [cite: 247]
    * **ENTONCES** el sistema asigna la bonificación al propietario para ese puesto y la muestra en la lista de "Bonificaciones asignadas". [cite: 248]

* **Escenario:** Propietario no encontrado
    * **DADO** que estoy en la pantalla de "Asignar bonificaciones"
    * **CUANDO** ingreso una cédula de identidad que no existe y hago clic en "Buscar propietario" [cite: 244]
    * **ENTONCES** el sistema muestra el mensaje "no existe el propietario". [cite: 250]
...
* **Escenario:** Propietario ya tiene bonificación en el puesto
    ...
    * **ENTONCES** el sistema muestra el mensaje "Ya tiene una bonificación asignada para ese puesto". [cite: 253]

* **Escenario:** Propietario deshabilitado
    * **DADO** que busco un propietario cuyo estado es "Deshabilitado"
    * **CUANDO** intento asignarle una bonificación
    * **ENTONCES** el sistema muestra el mensaje "El propietario esta deshabilitado. No se pueden asignar bonificaciones". [cite: 255]
...
* **Escenario:** Cambio de estado exitoso
    * **DADO** que he buscado un propietario existente por su cédula [cite: 272]
    * **CUANDO** selecciono un nuevo estado (diferente al actual) de la lista y hago clic en "Cambiar estado" [cite: 276]
    * **ENTONCES** el sistema actualiza el estado del propietario, le envía una notificación, y el nuevo estado se refleja en pantalla. [cite: 277]
...
* **Escenario:** Propietario no encontrado
    ...
    * **ENTONCES** el sistema muestra el mensaje "no existe el propietario". [cite: 281, 282]

* **Escenario:** El estado seleccionado es el mismo que el actual
    * **DADO** que he buscado un propietario existente
    * **CUANDO** selecciono el mismo estado que ya tiene y hago clic en "Cambiar estado" [cite: 276]
    * **ENTONCES** el sistema muestra el mensaje "El propietario ya esta en estado " seguido del nombre del estado. [cite: 283]